...
# #plt.plot(a_runid,a_nrofev,'o')
    p = np.poly1d(z)
    axs[0].plot(a_expo,a_nrofev,'.',c=col,label='LLfree-LLnull < ' +str(delta))
    axs[0].plot(a_expo,p(a_expo),'-',c=col)
    axs[0].legend()
    axs[1].plot(a_expo,(a_nrofev-p(a_expo))/np.max(a_nrofev),'.',c=col)
...
#    plt.figure()
    ...
    ax1.errorbar(a_exps, a_evs, yerr=a_evs_err,fmt='o')
    ...
    ax2.set_xticklabels(a_runidshowed.astype(str))
 
#     divisionfactor = len(new_tick_locations)/15
#     new_tick_locations = new_tick_locations[::divisionfactor]
#     a_runid = a_runid.astype(int)
#     a_runidshowed = a_runid[::divisionfactor]
#     ax2.set_xlim(0, expomax) 
#     print 'a_exps[::divisionfactor] = ', a_exps[::divisionfactor]
#     ax2.set_xticks(a_exps[::divisionfactor])

#    ax2.set_xticklabels(['7','8','99'])

#    ax2.set_xticks(a_exps[::divisionfactor])
#    ax2.set_xticklabels(a_runidshowed.astype(str))
#   ax2.tick_params(labelleft=False, labelbottom=False,labelsize=0)    
#    ax2.set_xticks(new_tick_locations)
#    ax2.set_xticklabels(a_runid.astype(str))
